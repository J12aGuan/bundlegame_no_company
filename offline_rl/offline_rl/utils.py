...
def softmax(values: list[float], temperature: float = 1.0) -> list[float]:
    ...
    exps = [math.exp(max(-40.0, min(40.0, (value - max_value) / temp))) for value in values]
    denom = sum(exps)
    return [value / denom for value in exps] if denom > 0 else [1.0 / len(values)] * len(values)

...
def validate_policy_training_schema(
    ...
    if not policy_training_rows:
        errors.append("policy_training.csv has no rows")
        return SchemaValidationResult(False, errors, warnings, 0, 0, 0)

    header = set(policy_training_rows[0].keys())
    if not any(column in header for column in PARTICIPANT_ID_COLUMNS):
        errors.append(
            "policy_training.csv missing participant identifier column: participant_id or publication_participant_id"
        )
    ...
    for state_id, rows in grouped.items():
        ...
        done_values = {to_int(row.get("done")) for row in rows}
    ...
    if snapshot:
        ...
        if expected_rows is not None and int(expected_rows) != len(policy_training_rows):
            warnings.append(
                f"dataset_snapshot policy_training_rows={expected_rows}, file rows={len(policy_training_rows)}"
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate BundleGame offline-RL input schema")
    parser.add_argument("--policy-training", required=True, help="Frozen policy_training.csv")
    parser.add_argument("--dataset-snapshot", default="", help="Frozen dataset_snapshot.json")
    return parser

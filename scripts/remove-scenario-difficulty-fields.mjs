...
function cleanScenarioEntry(entry = {}) {
  ...
  delete cleanedMetadata.targetDifficulty;

  const cleanedScenarios = Array.isArray(entry.scenarios)
    ? entry.scenarios.map((scenario = {}) => {
        const next = { ...scenario };
        delete next.classification;
        delete next.score_gap;
        delete next.relative_gap;
        return next;
      })
    : [];
...
async function main() {
  ...
  console.log(`Cleaned ${updatedCount} scenario dataset entries.`);
}

main().catch((error) => {
  console.error("Failed to remove scenario difficulty fields:", error);
  process.exit(1);
});
